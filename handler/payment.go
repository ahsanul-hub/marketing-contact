...
func CreateTransactionVa(c *fiber.Ctx) error {
	...
	case "va_bca":
		res, err := lib.RequestBcaWinpay(transactionID, &transaction)
		if err != nil {
			log.Println("Generate va failed:", err)
			return c.Status(fiber.StatusInternalServerError).JSON(fiber.Map{
				"success": false,
		...
		vaPayment := http.VaPayment{
			VaNumber:      res.VirtualAccountData.VirtualAccountNo,
			TransactionID: transactionID,
			Bank:          "BCA",
			ExpiredDate:   res.VirtualAccountData.ExpiredDate,
		}

		VaTransactionCache.Set(res.VirtualAccountData.VirtualAccountNo, vaPayment, cache.DefaultExpiration)

		return response.ResponseSuccess(c, fiber.StatusOK, fiber.Map{
			"success":        true,
			"va":             res.VirtualAccountData.VirtualAccountNo,
	...
	case "va_bri":
		res, err := lib.RequestBriWinpay(transactionID, &transaction)
		if err != nil {
			log.Println("Generate va failed:", err)
			return c.Status(fiber.StatusInternalServerError).JSON(fiber.Map{
				"success": false,
	...
	case "va_permata":
		res, err := lib.RequestPermataWinpay(transactionID, &transaction)
		if err != nil {
			log.Println("Generate va failed:", err)
			return c.Status(fiber.StatusInternalServerError).JSON(fiber.Map{
				"success": false,
	...
	case "va_mandiri":
		res, err := lib.RequestMandiriWinpay(transactionID, &transaction)
		if err != nil {
			log.Println("Generate va failed:", err)
			return c.Status(fiber.StatusInternalServerError).JSON(fiber.Map{
				"success": false,
	...
	case "va_bni":
		res, err := lib.RequestBniWinpay(transactionID, &transaction)
		if err != nil {
			log.Println("Generate va failed:", err)
			return c.Status(fiber.StatusInternalServerError).JSON(fiber.Map{
				"success": false,
